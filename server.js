...
if (process.env.GOOGLE_KEY_JSON) {
  try {
    ...
    console.log('✅ Archivo de clave de Google creado exitosamente');
  } catch (error) {
    console.error('❌ Error creando archivo de clave de Google:', error);
  }
} else {
  console.log('⚠️ GOOGLE_KEY_JSON no encontrada en variables de entorno');
}
...
app.get('/api/sheets', async (req, res) => {
  try {
    ...
    console.log('🔑 Usando archivo de clave:', process.env.GOOGLE_KEYFILE);
    
    // Verificar que el archivo de clave existe
    if (!process.env.GOOGLE_KEYFILE || !fs.existsSync(process.env.GOOGLE_KEYFILE)) {
      console.error('❌ Archivo de clave de Google no encontrado');
      return res.status(500).json({ error: 'Configuración de Google Sheets no válida' });
    }
...
app.get('/api/test', (req, res) => {
  res.json({ 
    ...
    googleKeyExists: !!process.env.GOOGLE_KEY_JSON,
    googleKeyFileExists: process.env.GOOGLE_KEYFILE ? fs.existsSync(process.env.GOOGLE_KEYFILE) : false
  });
});
